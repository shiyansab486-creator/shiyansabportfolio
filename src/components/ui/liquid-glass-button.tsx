...
function LiquidButton({
  ...
  return (
    <Comp data-slot="button" className={cn("relative isolate overflow-hidden rounded-full", liquidbuttonVariants({ variant, size, className }))} {...props}>
      <div className="absolute left-0 top-0 z-0 h-full w-full rounded-full shadow-[0_0_6px_rgba(0,0,0,0.03),0_2px_6px_rgba(0,0,0,0.08),inset_3px_3px_0.5px_-3px_rgba(255,255,255,0.45),inset_-3px_-3px_0.5px_-3px_rgba(255,255,255,0.35),inset_0_0_6px_6px_rgba(255,255,255,0.08),0_0_22px_rgba(59,130,246,0.2)] transition-all" />
      <div className="absolute left-0 top-0 isolate -z-10 h-full w-full overflow-hidden rounded-full" style={{ backdropFilter: 'url("#container-glass")' }} />
...
> = {
  ...
  primary: {
    outer: "bg-gradient-to-b from-[#0A0A0A] to-[#3B82F6]",
    inner: "bg-gradient-to-b from-[#DBEAFE] via-[#1D4ED8] to-[#8B5CF6]",
    button: "bg-gradient-to-b from-[#3B82F6] to-[#8B5CF6]",
    textColor: "text-white",
...
const metalButtonVariants = (variant: ColorVariant = "default", isPressed: boolean, isHovered: boolean, isTouchDevice: boolean) => {
  ...
  return {
    ...
    button: cn("relative z-10 m-[1px] inline-flex h-11 transform-gpu cursor-pointer items-center justify-center overflow-hidden rounded-md px-6 py-2 text-sm font-semibold leading-none outline-none will-change-transform", colors.button, colors.textColor, colors.textShadow),
    buttonStyle: {
  ...
  return (
    <div className={variants.wrapper} style={variants.wrapperStyle}>
      ...
      >
        ...
        {children || "Button"}
        {isHovered && !isPressed && !isTouchDevice && <div className="pointer-events-none absolute inset-0 rounded-lg bg-gradient-to-t from-transparent to-white/5" />}
      </button>
    </div>
  );
});
...
export { Button, buttonVariants, liquidbuttonVariants, LiquidButton, GlassFilter, MetalButton };