...
export const GlassEffect: React.FC<GlassEffectProps & { as?: "div" | "button" | "a" }> = ({
  children,
  className = "",
  style = {},
  href,
  ...
  const isInteractive = href || onClick || as === "button" || as === "a";
  const baseClass = isInteractive ? "cursor-pointer" : "";
  const contentClass = `relative flex overflow-hidden text-white transition-all duration-700 ${baseClass} ${className}`;
  ...
  const layers = (
    <>
      ...
      <div
        ...
        style={{
          boxShadow:
            "inset 0.5px 0.5px 1px 0 rgba(255, 255, 255, 0.3), inset -0.5px -0.5px 1px 0 rgba(255, 255, 255, 0.05)",
        }}
      />
      <div className="relative z-30 flex items-center justify-center gap-2 w-full h-full">{children}</div>
    </>
  );

  if (as === "div" || (!href && !onClick && !as)) {
    return (
      <div className={contentClass} style={glassStyle}>
        {layers}
      </div>
    );
  }
  ...
  return href ? (
    <a href={href} target={target} rel={rel} download={download} onClick={onClick} className={contentClass} style={glassStyle}>
      {layers}
    </a>
  ) : (
    <button type="button" onClick={onClick} className={contentClass} style={glassStyle}>
      {layers}
    </button>
  );
};
...
// Button Component
export const GlassButton: React.FC<{ children: React.ReactNode; href?: string; onClick?: () => void; className?: string; download?: boolean; target?: string; rel?: string; }> = ({
  children,
  href,
  onClick,
  className = "",
  ...
  <GlassEffect
    ...
    target={target}
    rel={rel}
    className={`rounded-3xl px-6 py-4 md:px-8 md:py-4 hover:px-9 hover:py-5 hover:rounded-4xl overflow-hidden ${className}`}
  >
    <div
      className="transition-all duration-700 hover:scale-95 text-white flex items-center justify-center gap-2"
      style={{
        transitionTimingFunction: "cubic-bezier(0.175, 0.885, 0.32, 2.2)",
      }}
    >
